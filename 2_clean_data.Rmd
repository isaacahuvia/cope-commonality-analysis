...
```{r pull session info}
sink(file = "2025.08.17_sessionInfo_for_cleaning_script.txt")
sessionInfo()
sink()
#Note - we ran into an error on macOS on 8/14/25 when using the settings specified in the "2025.08.14_sessionInfo_for_cleaning_script.txt" file. missForest is no longer running unless there are exactly two factors (we get an incorrect cutoff specified error)
```
